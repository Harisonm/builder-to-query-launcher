SELECT
  ...
  CAST(niv_agregation AS INT64) AS niveau_aggregation,
  ...
  CAST(NULL AS STRING) AS id_marque,
  CAST(UPPER(librayon) AS STRING) AS librayon,
  ...
  CAST(UPPER(libfammkg) AS STRING) AS type_produit,
  CAST(NULL AS STRING) AS ref_id_produit,
  CAST(UPPER(cmarque) AS STRING) AS marque_full,
  REPLACE('{kpi_group_name}','_',' ') AS kpi_group,
  REPLACE('{field_name}','_',' ') AS kpi,
  ref_client.kpi_code AS kpi_ordre,
  CAST(nb_clts AS INT64) AS nb_clients_total,
  CAST(SAFE_DIVIDE(SAFE_MULTIPLY({field_name},nb_clts),100) AS INT64) AS kpi_valeur
FROM
  `plateforme-fournisseurs-prod.data.clients_darty_{niv}__all_versions` AS client LEFT JOIN `plateforme-fournisseurs-prod.referential_table.kpi_codes` as ref_client
