...
new_users AS (
  ...
  CROSS JOIN  (SELECT MAX(update_date) last_update_date FROM `plateforme-fournisseurs-prod.d_extract_tableau.insights_ventes_geo_production` ) last_update
...
FROM
  insights_ventes_geo JOIN new_users ON LOWER(TRIM(new_users.user))=LOWER(TRIM(insights_ventes_geo.user))