WITH
  ...
  insights_stock_last_day_fournisseur_converteo_fnacdarty_restricted AS (
  SELECT
    stocks.*,
    fournisseur.user
  FROM
  ...
  JOIN
    `plateforme-fournisseurs-prod.c_dashboard.insights_stock_last_twelve_months` AS stocks
  ON
    stocks.id_fournisseur = fournisseur.id_fournisseur )
SELECT
  DISTINCT *
FROM
  insights_stock_last_day_fournisseur_converteo_fnacdarty_restricted