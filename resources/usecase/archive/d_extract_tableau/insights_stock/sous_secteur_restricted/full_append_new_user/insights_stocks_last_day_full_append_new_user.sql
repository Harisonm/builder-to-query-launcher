...
new_users AS (
  ...
  CROSS JOIN  (SELECT MAX(update_date) last_update_date FROM `plateforme-fournisseurs-prod.d_extract_tableau.insights_stocks_last_day`) last_update
...
FROM
  insights_stock_last_day JOIN new_users ON LOWER(TRIM(new_users.user))=LOWER(TRIM(insights_stock_last_day.user))