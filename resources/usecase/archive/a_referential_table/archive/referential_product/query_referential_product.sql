  ...
  UNION ALL (
    WITH
      ...
      WHERE
        ...
        AND import_date='{date_begin}')
    SELECT
      *
    FROM
      niv0_ref_ecommerce ) )
SELECT
  DISTINCT niv0_referential_product.*
FROM
  niv0_referential_product
LEFT JOIN
  `plateforme-fournisseurs-prod.referential_table.referential_product` AS ref_product_preview
USING
  (import_date,
   ...
   id_fournisseur)
WHERE
  ref_product_preview.product_id_formatted IS NULL