CREATE TABLE `plateforme-fournisseurs-prod.datamart.insights_ecommerces_mot_cle` PARTITION BY date AS (
    SELECT
    day AS date,
    SAFE_CAST(product_id AS STRING) AS id_produit,
    SAFE_CAST('DARTY' AS STRING) AS enseigne,
  ...
  FROM
    `plateforme-fournisseurs-prod.data.darty_search` AS darty_search
  WHERE
     product_id IS NOT NULL
    AND product_id!=''
    AND UPPER(request) != 'UNDEFINED'
