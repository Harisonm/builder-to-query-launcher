WITH
  ...
  FROM
    `plateforme-fournisseurs-prod.c_dashboard.insights_ventes_geo` AS ventes_geo
  WHERE
    date BETWEEN '2021-01-01'
    AND CURRENT_DATE()
    AND secteur = 'TVV'
    AND REGEXP_CONTAINS(nom_magasin, r'FNAC([0-9]+)') IS NOT TRUE
    AND REGEXP_CONTAINS(nom_magasin, r'FNAC([0-9]+)') IS NOT TRUE
    AND REGEXP_CONTAINS(nom_magasin, r'DARTY([0-9]+)') IS NOT TRUE
    AND REGEXP_CONTAINS(nom_magasin, r'DARTY([0-9]+)') IS NOT TRUE
  ...
  FROM
    base
  WHERE
    MOD(CAST(RAND()*100 AS INT64), 100) = 1 ),
  marques AS (
  SELECT
