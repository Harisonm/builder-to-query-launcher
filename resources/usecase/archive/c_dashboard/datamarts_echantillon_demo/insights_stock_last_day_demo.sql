WITH
  base AS (
  SELECT
    insights_stock_last_day.* EXCEPT(enseigne,
      canal_2),
    (
      CASE enseigne
  ...
  FROM
    `plateforme-fournisseurs-prod.c_dashboard.insights_stock_last_day` AS insights_stock_last_day
  WHERE
  ...
  FROM
    base
  WHERE
    MOD(CAST(RAND()*1 AS INT64), 100) = 1 ),
  marques AS (
  SELECT
    DISTINCT id_marque
  FROM
